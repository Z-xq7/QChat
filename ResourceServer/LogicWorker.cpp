...
void LogicWorker::RegisterCallBacks()
{
	_fun_callbacks[ID_TEST_MSG_REQ] = [this](shared_ptr<CSession> session, const short& msg_id,
		const string& msg_data) {
			...
			LOG_DEBUG("Received test data: " << data);
	...
	_fun_callbacks[ID_UPLOAD_FILE_REQ] = [this](shared_ptr<CSession> session, const short& msg_id,
		const string& msg_data) {
			...
			LOG_DEBUG("File [" << name << "] hash value: " << hash_value << ", worker index: " << index);
	...
	//�ͻ����ϴ�ͷ��
	_fun_callbacks[ID_UPLOAD_HEAD_ICON_REQ] = [this](shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
		{
			Json::Reader reader;
			Json::Value root;
			reader.parse(msg_data, root);
			...
			size_t hash_value = hash_fn(name); // ���ɹ�ϣֵ
			int index = hash_value % FILE_WORKER_COUNT;
			LOG_DEBUG("Head icon [" << name << "] hash value: " << hash_value << ", worker index: " << index);
	...
	_fun_callbacks[ID_DOWN_LOAD_FILE_REQ] = [this](shared_ptr<CSession> session, const short& msg_id,
		const string& msg_data) {
			...
			size_t hash_value = hash_fn(name); // ���ɹ�ϣֵ
			int index = hash_value % FILE_WORKER_COUNT;
			LOG_DEBUG("Download file [" << name << "] hash value: " << hash_value << ", worker index: " << index);
	...
	_fun_callbacks[ID_IMG_CHAT_UPLOAD_REQ] = [this](shared_ptr<CSession> session, const short& msg_id,
		const string& msg_data) {
			...
			size_t hash_value = hash_fn(name); // ���ɹ�ϣֵ
			int index = hash_value % FILE_WORKER_COUNT;
			LOG_DEBUG("Upload image chat file [" << name << "] hash value: " << hash_value << ", worker index: " << index);
			...
			FileSystem::GetInstance()->PostMsgToQue(
				std::make_shared<FileTask>(session, ID_IMG_CHAT_UPLOAD_REQ, uid, file_path_str, name, seq, total_size,
					trans_size, last, file_data, callback, message_id, sender, receiver), index );
		};
	...
	//�ͻ�����������ͼƬ��������������Ϣid��ѯ�����Ϣ���ļ�Ͷ�ݵ��ļ�����ϵͳ�������أ�������ɺ������֪ͨ�ͻ��˽�����һ������
	_fun_callbacks[ID_IMG_CHAT_DOWN_REQ] = [this](std::shared_ptr<CSession> session, const short& msg_req_id,
		const string& msg_data) {
			...
			LOG_DEBUG("Hash value: " << hash_value << ", Index: " << index);
	...
	_fun_callbacks[ID_FILE_CHAT_DOWN_REQ] = [this](std::shared_ptr<CSession> session, const short& msg_req_id,
		const string& msg_data) {
			Json::Reader reader;
			Json::Value root;
			reader.parse(msg_data, root);
			...
			auto callback = [=](const Json::Value& result) {
				...
				rtvalue["receiver_id"] = receiver;
				std::string return_str = rtvalue.toStyledString();
				session->Send(return_str, ID_FILE_CHAT_DOWN_RSP);
				};
			...
			int index = hash_value % DOWN_LOAD_WORKER_COUNT;
			LOG_DEBUG("File [" << name << "] Hash value: " << hash_value << ", Index: " << index);
...
{
	LOG_DEBUG("Received message id: " << task->_recvnode->_msg_id);
	auto call_back_iter = _fun_callbacks.find(task->_recvnode->_msg_id);
	if (call_back_iter == _fun_callbacks.end()) {
		return;
	}

	// 从 RecvNode 的 vector<char> 构造 std::string
	std::string msg_data;
	if (!task->_recvnode->_data.empty() && task->_recvnode->_cur_len > 0) {
		msg_data.assign(task->_recvnode->_data.data(),
			std::min(task->_recvnode->_cur_len, static_cast<std::size_t>(task->_recvnode->_data.size())));
	}

	call_back_iter->second(task->_session, task->_recvnode->_msg_id, msg_data);
}