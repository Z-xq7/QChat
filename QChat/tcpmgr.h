...
signals:
    ...
    //发送文件消息后服务器回传接收到消息的信号后的通知
    void sig_chat_file_rsp(int thread_id, std::shared_ptr<FileChatData> chat_data);
    //聊天文本变化->接收对方发来的文本消息
    ...
    //聊天文件变化->接收对方发来的文件消息
    void sig_file_chat_msg(std::shared_ptr<FileChatData> msg_list);
